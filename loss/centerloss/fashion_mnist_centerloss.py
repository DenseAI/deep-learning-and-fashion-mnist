...
from keras.layers import Input, Dense, Flatten, BatchNormalization, MaxPooling2D, Dropout, add
...
def my_model(input, labels, kernels=[3,3],droprate=0.25, factor=1):
    #x = BatchNormalization()(x)
    outs = []
    for kernel in kernels:
        x = Conv2D(32, (kernel, kernel), activation='relu', padding='same')(input)
...
def run(lambda_centerloss):
    ...
    kernels = 1
    factor = 1
    model_name = "centerloss_kernel_{}_k_{}".format(str(kernels), factor)
    loss_value = 'val_loss'
