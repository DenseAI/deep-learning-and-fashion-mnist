...
class SphereFace(Layer):
    def __init__(self, n_classes=10, s=30.0, m=6, regularizer=None, **kwargs):
...
def create_model():
    ...
    spherefaces_output = SphereFace(10)([x, input_label])
    softmax_output = Dense(num_classes, activation='softmax')(x)
    model = Model([input_img, input_label], [spherefaces_output, softmax_output])
...
model_train_history = model.fit([x_train_with_channels, y_train_categorical] , [y_train_categorical,y_train_categorical],
                                batch_size=batch_size,
                                epochs=epochs,
                                verbose=1,
                                validation_data=([x_test_with_channels, y_test_categorical], [y_test_categorical,y_test_categorical]),
